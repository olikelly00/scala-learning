...
class ReceiptPrinterSpec extends AnyWordSpec with Matchers {
  ...
  "A ReceiptPrinter" should {
    "format a receipt" which {
      ...
      "contains the prices from the cafe" in {
        val printer = new ReceiptPrinter(
          coffeeConnectionCafe,
          Map("Cafe Latte" -> 1)
        )
        printer.receipt should include ("Muffin Of The Day")
        printer.receipt should include ("Chocolate Chip Muffin")
        printer.receipt should include ("Cortado")
        printer.receipt should include ("6.40")
        printer.receipt should include ("14.80")
        printer.receipt should include ("4.55")
      }
    }
  }
}
